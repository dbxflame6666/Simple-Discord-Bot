...
class withdraw(commands.Cog):
    ...
    @bridge.bridge_command(
        name='withdraw',
        description='Withdraw your current Bank cash to cash on Hand',
        aliases=['with']
        )
    async def withdraw(self, ctx, *, amount=None):
        ...
        emErr.add_field(
            name='Current Coins on your Bank:',
            value=f'{eco} {bank:,}'
        )
        ...
        if str(amount) == 'all':
            ...
            emA = discord.Embed(
                description=f'You succesfully withdrawed {eco} **{bank:,}**.',
            ...
            async with aiosqlite.connect(self.DB) as db:
                async with db.execute("UPDATE economy SET cash = cash + bank, bank = 0 WHERE user_id = ?", (ctx.author.id,)) as cursor:
                    await db.commit()
            ...
            em = discord.Embed(
                description=f'You succesfully withdrawed {eco} **{amount}**.',
                color=discord.Color.green(),
                timestamp=datetime.now()
            )
